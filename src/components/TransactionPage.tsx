...
import { TableContextConsumer } from './Txprovider';
...
const TransactionPage: React.FC = () => {
  ...
  const headerCells: TableHeader[] = [
    ...
    { id: 'retry', alignment: TableAlignment.right, label: 'Retry' },
  ];
  const rows: TableRow[] = [
    {
      id: '1',
      cells: [
        { content: <EthHashInfo hash="0x7BF0C0259DA2db1Cc9A484945722221c5B800139" showCopyBtn shortenHash={4} /> },
        { content: <EthHashInfo hash="0x7BF0C0259DA2db1Cc9A484945722221c5B800139" showCopyBtn shortenHash={4} /> },
        { content: <Text size="sm">100.123456789 ETH</Text> },
        { content: <IconText iconSize="sm" textSize="md" iconType="alert" text="failed" /> },
        { alignment: TableAlignment.right, content: <Icon size="md" type="resync" /> },
      ],
    },
  ];
  ...
  return (
    <>
      <TableContextConsumer>{(appContext) => appContext && <div>{appContext}</div>}</TableContextConsumer>
      {/* <div>
      <Divider />
